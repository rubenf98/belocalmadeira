...
function ConfirmationTour({ match }) {
    ...
    useEffect(() => {
        ...
        axios.get(`${window.location.origin}/api/transfers/showFromToken?token=${token}`).then((response) => {
                    ...
                    (Object.keys(data).length === 0) ?
                        ...
                        <Fragment>
                            ...
                            <Summary type="flex" justify="flex-start">
                                ...
                                <Detail><span>{text.details.trajectory} </span> {data.trajectory} </Detail>
                                <Detail><span>{text.details.date} </span> {data.date} </Detail>

                                {data.return_date && <Detail><span>{text.details.return_date} </span> {data.return_date} </Detail>}
                            </Summary>
